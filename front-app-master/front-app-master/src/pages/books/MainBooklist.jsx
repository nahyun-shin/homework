...
function MainBookList() {
  ...
  const { data: newBooks, isLoading: loadingNew, error: errorNew } = useQuery({
    queryKey: ['main', 'new'],
    queryFn: () => bookAPI.getBooksByType('new'),
  });

  const { data: bannerBooks, isLoading: loadingBanner, error: errorBanner } = useQuery({
  ...
  if (!bannerBooks || bannerBooks.length === 0) return <div>배너 도서가 없습니다.</div>;
    ...
    <div className="main-container">
      <section className="banner-slider">
        {bannerBooks.map((book, index) => (
          <div
          key={book.bookId}
          className={`banner-slide ${index === validIndex ? "active" : "inactive"}`}
          >
              ...
              <div className="banner-slide-content">
                ...
                <div className="banner-slide-right">
                  ...
                  <button
                    ...
                    onClick={() => alert(`책갈피 클릭! bookId: ${book.bookId}`)}
                    aria-label="책갈피 버튼"
                    >
                    둘러보기
                  </button>
        ...
        <div className="indicator-container">
          {bannerBooks.map((_, index) => (
            <span
              key={index}
              className={`indicator-dot ${index === validIndex ? "active" : ""}`}
