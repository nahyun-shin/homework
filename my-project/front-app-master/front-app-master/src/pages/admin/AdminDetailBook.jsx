...
function DetailBook() {
  ...
  const prevImage = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? book.fileList.length - 1 : prev - 1
    );
  };
  const nextImage = () => {
    setCurrentIndex((prev) =>
      prev === book.fileList.length - 1 ? 0 : prev + 1
    );
  };
      ...
      <div className="content-bg">
        <div className="left-body">
          <div className="detail-img-wrap">
            <img
              src={book.fileList[currentIndex].imageUrl}
              alt={book.fileList[currentIndex].fileName}
            />
          </div>
          <div className="detail-img-button-wrap">
            <button onClick={prevImage}>이전</button>
            <span>
              {currentIndex + 1} / {book.fileList.length}
            </span>
        ...
        <div className="right-body">
          <div className="right-bg">
            <div className="detail-text-wrap">
              ...
              <p className="detail-pub">{book.writer}</p>
              <p className="detail-pub">{book.publisher}</p>
              <p className="detail-pub">{book.pubDate}</p>
            ...
            <div className="content-list-button-bg dt">
              ...
              <button type="button" onClick={goDelete} className="del-btn">
                삭제하기
              </button>
              <button type="button" className="" onClick={goBookList}>
                목록가기
              </button>
            </div>
          </div>
